    ...
    <div class="flex overflow-x-auto gap-16">
        ...
        @else
            @foreach ($cooperationImg as $image)
                <img class="w-12 md:w-fit" src="{{ asset('storage/' . $image->image) }}" />
            @endforeach
        @endif
    </div>
</div>
...
<section class="w-full overflow-hidden py-28">
    <div class="container grid grid-cols-1 md:grid-cols-2 items-center gap-12">
        <div class="space-y-4 lg:ml-20">
            ...
            @if (empty($abouts->title) && empty($abouts->content))
                <p class="text-center text-xneutral-200 font-montserrat text-lg">
                    No data available
                </p>    
            @else
                ...
                <div class="text-base sm:text-lg font-semibold text-xneutral-200 font-montserrat">
                    {!! $abouts->content !!} 
                </div>
                <a href="#"
        ...
        @else
            @foreach ($news as $list)
                <div class="p-[14px] rounded-[20px] border border-xneutral-100 bg-xneutral-0">
                    ...
                    <a href="{{ route('berita.show', $list->slug) }}"
...
<section class="container mt-28">
    ...
    <div class="grid grid-cols-2 gap-12 lg:grid-cols-4 mt-11 text-center">
        @if ($rectors->isEmpty())
            <div class="col-span-2 lg:grid-cols-4 mt-11 text-center">
                <p class="text-center text-xneutral-200 font-montserrat text-lg">
                    No data available
                </p>
            </div>
            ...
            @foreach ($rectors as $rector)
                <div class="flex flex-col items-center">
                    <div class="rounded-full overflow-hidden w-fit mb-6">
                        <img src="{{ asset('storage/' . $rector->image) }}" alt="{{ $rector->name }}" />
                    </div>
...
<section class="mt-28 x-announcement w-full">
    <div class="container pt-9 pb-16">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            @empty
                <div class="py-[26px] px-7 rounded-[20px] border border-xneutral-100 bg-white">
                    <p class="text-center text-xneutral-400 font-montserrat text-base">
                        No data available
                    </p>
                </div>
            @endforelse
